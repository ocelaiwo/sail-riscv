...
// ----- Local memory for rtl model

uint64_t lMASK = 0xFFFFFFul;
...
struct lblock *local_memory = NULL;

void lwrite_mem_byte(uint32_t address, uint8_t byte)
{
  uint64_t mask = address & ~lMASK;
  uint64_t offset = address & lMASK;

  struct lblock *current = local_memory;
   ...
   */
  struct lblock *new_block = (struct lblock *)malloc(sizeof(struct lblock));
  new_block->block_id = mask;
  new_block->mem = (uint8_t *)calloc(lMASK + 1, sizeof(uint8_t));
  new_block->mem[offset] = byte;
  new_block->next = local_memory;
  local_memory = new_block;
}

uint32_t lread_mem_byte(uint32_t address)
{
  uint64_t mask = address & ~lMASK;
  uint64_t offset = address & lMASK;
...
void lwrite_mem(uint32_t address, uint32_t value, uint8_t size) {
  switch (size) {
  case BYTE: {
    lwrite_mem_byte(address, (uint8_t) value);
    break;
  }
  case HALFWORD: {
    lwrite_mem_byte(address, (uint8_t) value);
    lwrite_mem_byte(address + 1, (uint8_t) (value >> 8));
    break;
  }
  case WORD: {
    lwrite_mem_byte(address, (uint8_t) value);
    lwrite_mem_byte(address + 1, (uint8_t) (value >> 8));
    lwrite_mem_byte(address + 2, (uint8_t) (value >> 16));
    lwrite_mem_byte(address + 3, (uint8_t) (value >> 24));
    break;
  }
  }
}

uint32_t lread_mem(uint32_t address, uint8_t size)
{
  uint32_t ret = 0;
  switch (size) {
  case BYTE: {
    ret = lread_mem_byte(address);
    break;
  }
  case HALFWORD: {
    ret = (lread_mem_byte(address + 1) << 8) + lread_mem_byte(address);
    break;
  }
  case WORD: {
    ret = (((((lread_mem_byte(address + 3) << 8) + lread_mem_byte(address + 2))
             << 8)
            + lread_mem_byte(address + 1))
           << 8)
        + lread_mem_byte(address);
    break;
  }
  }
  /* printf("address: (0x%08X)\n", address); */
  /* printf("bytes are: (0x%08X) (0x%08X) (0x%08X) (0x%08X)\n", lread_mem_byte(address), lread_mem_byte(address + 1), lread_mem_byte(address + 2), lread_mem_byte(address + 3)); */
...
/*   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, */
/*   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, */
/*   0x00, 0x00, 0x00, 0x00, 0x6f, 0x00, 0xa0, 0x18, 0x13, 0x00, 0x00, 0x00, */
...
/*   0x97, 0x01, 0x48, 0x00, 0x93, 0x81, 0x41, 0x72, 0x13, 0x05, 0x00, 0x40, */
/*   0x97, 0x05, 0x48, 0x00, 0x93, 0x85, 0x85, 0xd7, 0x17, 0x06, 0x48, 0x00, */
/*   0x13, 0x06, 0x06, 0xf3, 0x63, 0x0d, 0xb5, 0x00, 0x29, 0xa0, 0x14, 0x41, */
/*   0x94, 0xc1, 0x11, 0x05, 0x91, 0x05, 0xe3, 0x9c, 0xc5, 0xfe, 0x17, 0x06, */
/*   0x48, 0x00, 0x13, 0x06, 0x66, 0xf1, 0x97, 0x05, 0x48, 0x00, 0x93, 0x85, */
/*   0xe5, 0xf0, 0x21, 0xa0, 0x23, 0x20, 0x06, 0x00, 0x11, 0x06, 0xe3, 0x9d, */
/*   0xc5, 0xfe, 0x17, 0x01, 0x49, 0x00, 0x13, 0x01, 0xa1, 0xd3, 0x17, 0x05, */
/*   0x48, 0x00, 0x13, 0x05, 0x25, 0xef, 0x13, 0x86, 0x01, 0x88, 0xb3, 0x05, */
/*   0xa6, 0x40, 0x17, 0x07, 0x49, 0x00, 0x13, 0x07, 0x27, 0x92, 0x33, 0x02, */
/*   0xb7, 0x40, 0x92, 0x85, 0x17, 0x06, 0x48, 0x00, 0x13, 0x06, 0x46, 0xed, */
/*   0x29, 0xa0, 0x14, 0x41, 0x94, 0xc1, 0x11, 0x05, 0x91, 0x05, 0xe3, 0x1c, */
/*   0x07, 0x00, 0x85, 0x07, 0x23, 0x00, 0x67, 0x00, 0x83, 0xc3, 0x07, 0x00, */
/*   0x85, 0x07, 0x33, 0x8e, 0xff, 0x40, 0x23, 0x00, 0x77, 0x00, 0x63, 0x57, */
/*   0xc0, 0x05, 0x03, 0xc5, 0x07, 0x00, 0xa1, 0x07, 0x33, 0x88, 0xff, 0x40, */
/*   0x23, 0x00, 0xa7, 0x00, 0x83, 0xce, 0x97, 0xff, 0x23, 0x00, 0xd7, 0x01, */
/*   0x03, 0xcf, 0xa7, 0xff, 0x23, 0x00, 0xe7, 0x01, 0x83, 0xc5, 0xb7, 0xff, */
/*   0x23, 0x00, 0xb7, 0x00, 0x83, 0xc2, 0xc7, 0xff, 0x23, 0x00, 0x57, 0x00, */
/*   0x83, 0xc6, 0xd7, 0xff, 0x23, 0x00, 0xd7, 0x00, 0x03, 0xc6, 0xe7, 0xff, */
/*   0x23, 0x00, 0xc7, 0x00, 0x03, 0xc3, 0xf7, 0xff, 0x23, 0x00, 0x67, 0x00, */
/*   0xe3, 0x4d, 0x00, 0xfb, 0x23, 0x20, 0x02, 0x04, 0x01, 0x45, 0x82, 0x80, */
/*   0x37, 0x07, 0x70, 0x00, 0x89, 0x46, 0xb7, 0x07, 0x70, 0x00, 0x23, 0x00, */
/*   0xd7, 0x00, 0x8d, 0x42, 0xa3, 0x80, 0x57, 0x00, 0x37, 0x13, 0x00, 0x80, */
/*   0x85, 0x43, 0x23, 0x00, 0x73, 0x00, 0x23, 0x00, 0x73, 0x00, 0xe5, 0xbf, */
/*   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, */
/*   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x97, 0x02, 0xb8, 0xff, */
/*   0x93, 0x82, 0x42, 0xf9, 0x82, 0x82, 0x13, 0x00, 0x00, 0x00, 0x13, 0x00, */
...
unsigned char memory[1472];
unsigned int hello_bin_len = 1472;
...
int main(int argc, char **argv)
{
  ...
  for (size_t i = 0; i < inst_n; ++i) {
    ...
    memory[0x200 + (i * 4) + 1] = 0b00000000;

    memory[0x200 + (i * 4) + 2] &= 0b11110000;

    memory[0x200 + (i * 4) + 3] = 0b00000000;
  }

  for (size_t i = 0; i < hello_bin_len; ++i) {
    write_mem(i, memory[i]);
    lwrite_mem_byte(i, memory[i]);
  }
    ...
    if (clk_posedge) {
      ...
      case NONSEQ: {
        if (read_insts == inst_n) {
          ...
          if (top->imem_haddr == ADDR_TRAP_DEFAULT) {
            exit(1);
          }
          read_insts++;
          /* top->imem_hrdata = memread(top->imem_haddr, top->imem_hsize); */
          top->imem_hrdata = lread_mem(top->imem_haddr, top->imem_hsize);
    ...
    if (clk_posedge) {
      ...
      if (do_write) {
        ...
        if (haddr == 0x80001000) {
          ...
          lwrite_mem(haddr, top->dmem_hwdata, hsize);
        }
      }
      ...
      case NONSEQ: {
        if (!top->dmem_hwrite) {
          ...
          top->dmem_hrdata = lread_mem(top->dmem_haddr, top->dmem_hsize);
          top->dmem_hresp = 0;
          top->dmem_hready = 1;
        } else {
          do_write = true;
  ...
  rv_ram_size = UINT64_C(0x800000);
  zPC = 0x200;
  rv_htif_tohost = UINT64_C(0x80001000);
  ...
  size_t step_no = 0;
  while (step_no < inst_n) {
    sail_int sail_step;
    CREATE(sail_int)(&sail_step);
    CONVERT_OF(sail_int, mach_int)(&sail_step, step_no);
    auto stepped = zstep(sail_step);
    flush_logs();
    KILL(sail_int)(&sail_step);
    if (stepped) {
      step_no++;
